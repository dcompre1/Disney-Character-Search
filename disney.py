...
if exists == 0:
    ...
    disneydf = pd.DataFrame.from_dict(new_data, orient='index',
                                      columns=['name', 'films',
                                               'shortFilms', 'tvShows',
                                               'videoGames'])
    engine = db.create_engine('sqlite:///Disney.db')
    disneydf.to_sql('disney_info', con=engine,
                    if_exists='replace', index=False)

    # getting character information from database
    character_info = disney_funcs.retrieve_from_database(engine,
                                                         character_name)
    ...
    disney_funcs.print_info(character_name, films,
                            short_films, tv_shows, video_games)
